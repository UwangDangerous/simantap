<div class="card p-3">
    <form action="" method="post">
        ...
        <input type="text" name="kode_barang" id="kode_barang" class='form-control col-form-label-sm' placeholder="Kode Produk" autocomplete='off' value="<?= $produk["kode_barang"]; ?>">
        <small class="form-text text-danger"><?= form_error('kode_barang'); ?></small>
        
        <label for="nama_barang">Nama Produk</label>
        <input type="text" name="nama_barang" id="nama_barang" class='form-control col-form-label-sm' placeholder="Nama Produk" autocomplete='off' value="<?= $produk["nama_barang"]; ?>">
        ...
        <select name="id_ktg" id="id_ktg" class='form-control'>
            ...
            <?php foreach ($ktg as $k) : ?>
                <?php if($produk["id_ktg"] == $k['id_ktg']) : ?>
                    <option selected value="<?= $k['id_ktg'];?>"> <?= $k['nama_ktg']; ?> </option>
                <?php else : ?>
        ...
        <select name="id_unit" id="id_unit" class='form-control'>
            ...
            <?php foreach ($unit as $u) : ?>
                <?php if($produk["id_unit"] == $u['id_unit']) : ?>
